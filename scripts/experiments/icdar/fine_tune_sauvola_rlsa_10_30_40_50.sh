...
for t in ${training[*]}; do
  ...
  for i in ${weights_10[*]}; do
    params="experiment=fine_tune_cb55_AB1_train_20_run_unet.yaml
        ...
        name=fine_tune_sauvola_rlsa_real_cb55_AB1_${t}_unet_loss_no_weights_10pt_100e
        +model.backbone.path_to_weights=${i}
        logger.wandb.tags=[unet,AB1,${t},3-classes,fine-tune,100-epochs,no-weights,sauvola,binary_rlsa,rlsa_real,10-epoch-pt]
        logger.wandb.group=fine-tune-sauvola-rlsa-real-10pt-${t}"
  ...
  for i in ${weights_20[*]}; do
    params="experiment=fine_tune_cb55_AB1_train_20_run_unet.yaml
        trainer.devices=${devices}
        datamodule.train_folder_name=${t}
        name=fine_tune_sauvola_rlsa_real_cb55_AB1_${t}_unet_loss_no_weights_20pt_100e
        +model.backbone.path_to_weights=${i}
        logger.wandb.tags=[unet,AB1,${t},3-classes,fine-tune,100-epochs,no-weights,sauvola,binary_rlsa,rlsa_real,20-epoch-pt]
        logger.wandb.group=fine-tune-sauvola-rlsa-real-20pt-${t}"
    python run.py ${params}
    #          echo ${params}
  done
  ...
  for i in ${weights_30[*]}; do
    params="experiment=fine_tune_cb55_AB1_train_20_run_unet.yaml
        trainer.devices=${devices}
        datamodule.train_folder_name=${t}
        name=fine_tune_sauvola_rlsa_real_cb55_AB1_${t}_unet_loss_no_weights_30pt_100e
        +model.backbone.path_to_weights=${i}
        logger.wandb.tags=[unet,AB1,${t},3-classes,fine-tune,100-epochs,no-weights,sauvola,binary_rlsa,rlsa_real,30-epoch-pt]
        logger.wandb.group=fine-tune-sauvola-rlsa-real-30pt-${t}"
    python run.py ${params}
    #          echo ${params}
  done
  for i in ${weights_40[*]}; do
    params="experiment=fine_tune_cb55_AB1_train_20_run_unet.yaml
        trainer.devices=${devices}
        datamodule.train_folder_name=${t}
        name=fine_tune_sauvola_rlsa_real_cb55_AB1_${t}_unet_loss_no_weights_40pt_100e
        +model.backbone.path_to_weights=${i}
        logger.wandb.tags=[unet,AB1,${t},3-classes,fine-tune,100-epochs,no-weights,sauvola,binary_rlsa,rlsa_real,40-epoch-pt]
        logger.wandb.group=fine-tune-sauvola-rlsa-real-40pt-${t}"
    python run.py ${params}
    #        echo ${params}
  done
  ...
  for i in ${weights_50[*]}; do
    params="experiment=fine_tune_cb55_AB1_train_20_run_unet.yaml
        trainer.devices=${devices}
        datamodule.train_folder_name=${t}
        name=fine_tune_sauvola_rlsa_real_cb55_AB1_${t}_unet_loss_no_weights_50pt_100e
        +model.backbone.path_to_weights=${i}
        logger.wandb.tags=[unet,AB1,${t},3-classes,fine-tune,100-epochs,no-weights,sauvola,binary_rlsa,rlsa_real,50-epoch-pt]
        logger.wandb.group=fine-tune-sauvola-rlsa-real-50pt-${t}"
    python run.py ${params}
    #        echo ${params}
  done
done
