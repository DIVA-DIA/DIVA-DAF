...
for t in ${training[*]}; do
  ...
  for j in "${!weights_bb[@]}"; do
    params="experiment=fine_tune_cb55_AB1_train_20_run_unet.yaml
          ...
          name=test_sauvola_rlsa_new_3cl_cb55_AB1_${t}_unet_loss_no_weights_w_header_50pt_100e
          logger.wandb.tags=[unet,AB1,${t},3-classes,test,100-epochs,no-weights,3cl_new,rlsa_new_3cl,new_heuristic,50-epoch-pt,with_header]
          logger.wandb.group=test-sauvola-rlsa-new-3cl-w-header-50pt-${t}"
