...
weights_setA=("/net/research-hisdoc/experiments_lars_paul/lars_luca/experiments/sem_seg_synthetic_D60-setA_unet64_loss_no_weights/2022-11-10/09-11-02/checkpoints/epoch\=199/backbone.pth")
...
for j in "${!weights_setA[@]}"; do
  for i in "${!training_set[@]}"; do
    ...
    params_unet="experiment=fine_tune_cb55_AB1_train_20_run_unet64.yaml
        ...
        name=fine_tune_D60-SetA_cb55_AB1_${training_set[i]}_run_unet64
        model.backbone.path_to_weights=${weights_setA[j]}
        datamodule.train_folder_name=${training_set[i]}
        logger.wandb.tags=[unet64,AB1,${training_set[i]},fine-tune,D60-SetA,4-classes,baseline,${epochs[i]}-epochs,no-weights,best-jaccard]
