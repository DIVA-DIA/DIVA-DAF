...
weights_setC=("/net/research-hisdoc/experiments_lars_paul/lars_luca/experiments/sem_seg_synthetic_PC60-setC_unet64_loss_no_weights/2022-11-10/14-11-33/checkpoints/epoch\=177/backbone.pth")
...
for j in "${!weights_setC[@]}"; do
  for i in "${!training_set[@]}"; do
    ...
    params_unet="experiment=fine_tune_cb55_AB1_train_20_run_unet64.yaml
        ...
        name=fine_tune_PC60-SetC_cb55_AB1_${training_set[i]}_run_unet64
        model.backbone.path_to_weights=${weights_setC[j]}
        datamodule.train_folder_name=${training_set[i]}
        logger.wandb.tags=[unet64,AB1,${training_set[i]},fine-tune,PC60-SetC,4-classes,baseline,${epochs[i]}-epochs,no-weights,best-jaccard]
