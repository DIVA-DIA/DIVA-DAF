...
def test_init_datamodule(data_module_cropped):
    ...
    assert data_module_cropped.class_encodings == [0, 90, 180, 270]
    assert np.array_equal(data_module_cropped.class_weights, [1., 1., 1., 1.])
    assert data_module_cropped.mean == [0.7050454974582423, 0.6503181590413941, 0.5567698583877997]
    assert data_module_cropped.std == [0.3104060859619884, 0.3053311838884033, 0.28919611393432726]
